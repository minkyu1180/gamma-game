...
public class Stage2_1PortalScript : MonoBehaviour, IDataPersistence
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Player") && Input.GetAxisRaw("Vertical") == 1)
        {
            if (other.gameObject.GetComponent<Rigidbody2D>().velocity.magnitude <= 0.1f && !InputDecoder.isGameInScript)
            {
                ...
                string textLocation;
                if (didTrueClearStage2) textLocation = "Text/Stage2-1/Ending/AfterALLEnding";
                else
                {
                    if (didClearStage2)
                    {
                        if (didClearStage2_2Hidden) textLocation = "Text/Stage2-1/Ending/ClearAfterItemReEnding";
                        else                        textLocation = "Text/Stage2-1/Ending/ClearReEnding";
                    }
                    else
                    {
                        if (didClearStage2_1) textLocation = "Text/Stage2-1/Ending/ReEnding";
                        else                  textLocation = "Text/Stage2-1/Ending/Ending";
                    }
                }
