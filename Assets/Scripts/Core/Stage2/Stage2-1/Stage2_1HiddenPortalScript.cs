...
public class Stage2_1HiddenPortalScript : MonoBehaviour, IDataPersistence
{
    ...
    public void LoadData(GameData data)
    {
        ...
        this.didClearStage2_1 = data.didClearStage2_1;
        this.didClearStage2_2Hidden = data.didClearStage2_2Hidden;
    }

    public void SaveData(ref GameData data)
    {
        data.didClearStage2_1 = this.didClearStage2_1;
    }
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Player") && Input.GetAxisRaw("Vertical") == 1)
        {
            if (other.gameObject.GetComponent<Rigidbody2D>().velocity.magnitude <= 0.1f && !InputDecoder.isGameInScript)
            {
                ...
                string textLocation;
                if (didTrueClearStage2) textLocation = "Text/Stage2-1/Ending/Hidden/AfterALLEnding";
                else
                {
                    if (didClearStage2)
                    {
                        if (didClearStage2_2Hidden) textLocation = "Text/Stage2-1/Ending/Hidden/ClearAfterItemReEnding";
                        else                        textLocation = "Text/Stage2-1/Ending/Hidden/ClearReEnding";
                    }
                    else
                    {
                        // if (didClearStage2_1) textLocation = "Text/Stage2-1/Ending/Hidden/ReEnding";
                        // else                  textLocation = "Text/Stage2-1/Ending/Hidden/Ending";
                        textLocation = "Text/Stage2-1/Ending/Hidden/Error";
                    }
                }
