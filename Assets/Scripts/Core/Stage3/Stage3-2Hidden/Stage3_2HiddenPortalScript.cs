...
public class Stage3_2HiddenPortalScript : MonoBehaviour, IDataPersistence
{
    ...
    {
        data.didClearStage3_2Hidden = this.didClearStage3_2Hidden;
    }
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Player") && Input.GetAxisRaw("Vertical") == 1)
        {
            if (other.gameObject.GetComponent<Rigidbody2D>().velocity.magnitude <= 0.1f && !InputDecoder.isGameInScript)
            {
                ...
                string textLocation;
                if (didTrueClearStage3) textLocation = "Text/Stage3-2Hidden/Ending/AfterALLEnding";
                else
                {
                    if (didClearStage3_2Hidden) textLocation = "Text/Stage3-2Hidden/Ending/ClearAfterItemReEnding";
                    else textLocation = "Text/Stage3-2Hidden/Ending/ClearReEnding";
                }
