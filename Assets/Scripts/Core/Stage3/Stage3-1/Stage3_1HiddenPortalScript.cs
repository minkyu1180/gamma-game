...
public class Stage3_1HiddenPortalScript : MonoBehaviour, IDataPersistence
{
    ...
    */
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Player") && Input.GetAxisRaw("Vertical") == 1)
        {
            if (other.gameObject.GetComponent<Rigidbody2D>().velocity.magnitude <= 0.1f && !InputDecoder.isGameInScript)
            {
                ...
                string textLocation;
                if (didTrueClearStage3) textLocation = "Text/Stage3-1/Ending/Hidden/AfterALLEnding";
                else
                {
                    if (didClearStage3)
                    {
                        if (didClearStage3_2Hidden) textLocation = "Text/Stage3-1/Ending/Hidden/ClearAfterItemReEnding";
                        else                        textLocation = "Text/Stage3-1/Ending/Hidden/ClearReEnding";
                    }
                    else
                    {
                        textLocation = "Text/Stage3-1/Ending/Hidden/Error";
                    }
                }
    ...
    IEnumerator GoNextStage(string textLocation)
    {
        ...
        SceneManager.LoadScene("Stage 3-2Hidden");
    }
}
