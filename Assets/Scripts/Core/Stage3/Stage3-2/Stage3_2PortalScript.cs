...
public class Stage3_2PortalScript : MonoBehaviour, IDataPersistence
{
    ...
    public void LoadData(GameData data)
    {
        ...
        this.didClearStage3_2 = data.didClearStage3_2;
        this.didClearStage3_2Hidden = data.didClearStage3_2Hidden;
    }

    public void SaveData(ref GameData data)
    {
        data.didClearStage3_2 = this.didClearStage3_2;
    }
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Player") && Input.GetAxisRaw("Vertical") == 1)
        {
            if (other.gameObject.GetComponent<Rigidbody2D>().velocity.magnitude <= 0.1f && !InputDecoder.isGameInScript)
            {
                ...
                string textLocation;
                if (didTrueClearStage3) textLocation = "Text/Stage3-2/Ending/AfterALLEnding";
                else
                {
                    if (didClearStage3)
                    {
                        if (didClearStage3_2Hidden) textLocation = "Text/Stage3-2/Ending/ClearAfterItemReEnding";
                        else                        textLocation = "Text/Stage3-2/Ending/ClearReEnding";
                    }
                    else
                    {
                        if (didClearStage3_2) textLocation = "Text/Stage3-2/Ending/ReEnding";
                        else                  textLocation = "Text/Stage3-2/Ending/Ending";
                    }
                }
